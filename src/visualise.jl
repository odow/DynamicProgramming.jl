...
function adddata!(output::Dict{String, Any}, plot_data::Function, results::Dict{Symbol, Any})
	output["data"] = Vector{Float64}[]
	for i=1:results[:n_replications]
		push!(output["data"], Float64[])
		y = 0.0
		for j=1:results[:n_stages]
        ...
        if it.head == :tuple
			if length(it.args) == 2
				...
				elseif it.args[2].head == :(=)
					add_to_output!(output, it.args[2].args[1],it.args[2].args[2])
				end
